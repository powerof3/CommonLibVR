...
namespace RE
{
	...
	void BSAudioManager::BuildSoundDataFromEditorID(BSSoundHandle& a_soundHandle, const char* a_editorID, UInt32 a_flags)
	{
		using func_t = decltype(&BSAudioManager::BuildSoundDataFromEditorID);
		REL::Offset<func_t> func(Offset::BSAudioManager::BuildSoundDataFromEditorID);
