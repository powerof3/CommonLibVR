...
namespace RE
{
	namespace BSScript
	{
		...
		inline void PackValue(Variable* a_dst, T&& a_src)
		{
			PackHandle(a_dst, std::forward<T>(a_src), static_cast<VMTypeID>(decay_pointer_t<U>::VMTYPEID));
		}
		...
		[[nodiscard]] inline T UnpackValue(const Variable* a_src)
		{
			return static_cast<T>(UnpackHandle(a_src, static_cast<VMTypeID>(decay_pointer_t<T>::VMTYPEID)));
		}
