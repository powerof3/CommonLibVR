...
namespace SKSE
{
	namespace Impl
	{
		...
		{
		public:
			using Key = std::variant<std::string, UInt32, UInt8>;
		...
		public:
			...
			void SendEvent(Key a_key, Args... a_args)
			{
			...
			void QueueEvent(Key a_key, Args... a_args)
			{
		...
		private:
			...
			void SendEvent_Tuple(Key a_key, Tuple&& a_tuple, std::index_sequence<I...>)
			{
